...
jQuery(function ($) {
  ...
  var $searchbox = $('#agentcode.search-agent-input');
  var availableAgents = [];
  $.ajax({
  ...
  $("input#agentname").on("change", function(){
  	/*selectedAgent = $(this).val().toUpperCase();
  	$.each( availableAgents.name, function(key,value){
  		var agentName = value.toUpperCase();
  		if( selectedAgent == agentName) { 
  			//$('input#agentid').val(availableAgents.id[key]);
  		}
  	});*/
  });
