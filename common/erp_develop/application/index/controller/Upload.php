...
class Upload extends Base
{
    ...
    /**
     * 文件上传包含图片,压缩,WORD,PSD等
     * 韩
     */
    public function uploadFileAll(){
    	...
    	if($file){
    		...
    		$temp_name = str_replace('tmp',$image_name,$temp_name);
    		$url = config('soaupload')['ip'].':'.config('soaupload')['port'];
    		$result = help::curlImages($info->getPathname(), $url."/index/uploadAll");
    		...
    		if($result){
    			...
    			$d['data'] = '/static/uploads/file/'.$info->getSaveName();
    			$d['code'] = 200;
    			$d['get'] = $_GET;
    			echo json_encode($d);
    		}
