...
class Product extends Base
{
    ...
    //应付供应商类型
    public  $types = ['a'=>'index_public_select',2=>'index_finance_showReceivableManage_hotel',3=>'index_finance_showReceivableManage_dining',4=>'index_finance_showReceivableManage_flight',5=>'index_finance_showReceivableManage_cruise',6=>'index_finance_showReceivableManage_visa',7=>'index_finance_showReceivableManage_scenic_spot',8=>'index_finance_showReceivableManage_vehicle',9=>'index_finance_showReceivableManage_tourguide',10=>'index_finance_showReceivableManage_singlesource',11=>'index_finance_showReceivableManage_ownexpense',12=>'index_finance_showReceivableManage_other'];  //1=>'团队产品',
    //应收供应商类型
    public  $types2 = ['a'=>'index_public_select',1=>'index_finance_showReceivableManage_teamproduct',2=>'index_finance_showReceivableManage_hotel',3=>'index_finance_showReceivableManage_dining',4=>'index_finance_showReceivableManage_flight',5=>'index_finance_showReceivableManage_cruise',6=>'index_finance_showReceivableManage_visa',7=>'index_finance_showReceivableManage_scenic_spot',8=>'index_finance_showReceivableManage_vehicle',9=>'index_finance_showReceivableManage_tourguide',10=>'index_finance_showReceivableManage_singlesource',11=>'index_finance_showReceivableManage_ownexpense',12=>'index_finance_showReceivableManage_other'];
    ...
    */
    ...
    public function TemplateUpdateAjax(){
        ...
        //拿到修改前的线路模板信息
        $old_route_template = $this->callSoaErp('post','/branchcompany/getBranchProductByRouteTemplateId',['route_template_id'=>$d['route_template_id']])['data'];
        $old_use_company_id = [];
        if (!empty($old_route_template))
        {
            $old_use_company_id = array_column($old_route_template, 'company_id');
        }

        //新增 190328
        $use_company_id = Arrays::get($param,'use_company_id');
        $d['use_company_id'] = '*';
        if($use_company_id){
        ...
        $d['before_days'] = Arrays::get($param,'before_days');
        ...
        foreach($journey_the_days as $k=>$v){
            ...
            $ar['route_journey_zone'] = empty($country_id[$k]) ? '' : $country_id[$k];
            $ar['status'] = 1;
...
//        $d['route_return_receipt_info'] = [];
        ...
        $OneBitePriceCurrency = Arrays::get($param,'OneBitePriceCurrency');  //一口价货币
        $route_once_price_id = Arrays::get($param,'route_once_price_id'); // 一口价主键ID
        ...
        $d['add_allocation'] = [];
        //酒店
        $d = $this->setTemplateAllocationInfoUpdate($d,2,$session_hotel,'room_name',$tourPrice,$tourCurrency);
        //用餐
        $d = $this->setTemplateAllocationInfoUpdate($d,3,$session_dining,'dining_name',$tourPrice,$tourCurrency);
        //航班
        $d = $this->setTemplateAllocationInfoUpdate($d,4,$session_flight,'flight_number',$tourPrice,$tourCurrency);
        //邮轮
        $d = $this->setTemplateAllocationInfoUpdate($d,5,$session_cruise,'cruise_name',$tourPrice,$tourCurrency);
        //签证
        $d = $this->setTemplateAllocationInfoUpdate($d,6,$session_visa,'visa_name',$tourPrice,$tourCurrency);
        //景点
        $d = $this->setTemplateAllocationInfoUpdate($d,7,$session_scenic_spot,'scenic_spot_name',$tourPrice,$tourCurrency);
        //车辆
        $d = $this->setTemplateAllocationInfoUpdate($d,8,$session_vehicle,'vehicle_name',$tourPrice,$tourCurrency);
        //导游
        $d = $this->setTemplateAllocationInfoUpdate($d,9,$session_tourGuide,'tour_guide_name',$tourPrice,$tourCurrency);
        //单项资源
        $d = $this->setTemplateAllocationInfoUpdate($d,10,$session_singleSource,'single_source_name',$tourPrice,$tourCurrency);
        //自费项目
        $d = $this->setTemplateAllocationInfoUpdate($d,11,$session_Optional,'own_expense_name',$tourPrice,$tourCurrency);
...
//        var_dump($d);
        ...
        if($settlement_type==1){
            foreach($OneBitePrice as $ky=>$OneBitePriceV){
                ...
                if(empty($route_once_price_id[$ky])){
                    $d['add_once_price'][] = $ar;
                }else{
                    $ar['route_once_price_id'] = $route_once_price_id[$ky];
        ...
        $result = $this->callSoaErp('post','/product/updateRouteTemplateByRouteTemplateId',$d);


        if ($result['code'] == 200)
        {
            //将此线路模板下的代售线路类型的产品status变为0
            //$this->callSoaErp('post','/branchcompany/updateBranchProductStatusByRouteTemplateId',['route_template_id' => $d['route_template_id'], 'branch_product_type_id' => -1, 'status'=>0]);

            $branch_array['branch_product_name'] = $d['route_name'];  //名称
            $branch_array['branch_product_begin_time'] = strtotime('2020-01-01');  //开始日期
            $branch_array['branch_product_end_time'] = strtotime('2020-01-01');  //开始日期
            $branch_array['branch_product_type_id'] = '-1';
            $branch_array['status'] = 1;

            $use_company_id = explode(',', $use_company_id);

            foreach ($use_company_id as $v)
            {
                if (!in_array($v,$old_use_company_id))
                {
                    $branch_array['user_company_id'] = $v;
                    $route_template = $this->callSoaErp('post','/product/getRouteTemplate',['route_template_id' => $d['route_template_id'], 'can_watch_company_id' => $v])['data'][0];
                    $branch_array['route_template_array'][0]['route_number'] = $route_template['route_number'];
                    $branch_array['route_template_array'][0]['price_currency_id'] = $route_template['currency_id'];
                    $branch_array['route_template_array'][0]['distributor_price'] = $route_template['once_price'];
                    $branch_array['route_template_array'][0]['customer_price'] = $route_template['once_price'];

                    $this->callSoaErp('post','/branchcompany/addBranchProductBigArray',$branch_array);
                }
            }
        }
     ...
     * @return mixed 返回写入数据
     */
    public function setTemplateAllocationInfoUpdate($d,$supplier_type_id,$source_data,$source_ky,$tourPrice,$tourCurrency){
...
//        var_dump($session_hotel);exit;
        ...
        //酒店
        $d = $this->setTemplateAllocationInfo($d,2,$session_hotel,'room_name',$tourPrice,$tourCurrency);
        //用餐
        $d = $this->setTemplateAllocationInfo($d,3,$session_dining,'dining_name',$tourPrice,$tourCurrency);
        //航班
        $d = $this->setTemplateAllocationInfo($d,4,$session_flight,'flight_number',$tourPrice,$tourCurrency);
        //邮轮
        $d = $this->setTemplateAllocationInfo($d,5,$session_cruise,'cruise_name',$tourPrice,$tourCurrency);
        //签证
        $d = $this->setTemplateAllocationInfo($d,6,$session_visa,'visa_name',$tourPrice,$tourCurrency);
        //景点
        $d = $this->setTemplateAllocationInfo($d,7,$session_scenic_spot,'scenic_spot_name',$tourPrice,$tourCurrency);
        //车辆
        $d = $this->setTemplateAllocationInfo($d,8,$session_vehicle,'vehicle_name',$tourPrice,$tourCurrency);
        //导游
        $d = $this->setTemplateAllocationInfo($d,9,$session_tourGuide,'tour_guide_name',$tourPrice,$tourCurrency);
        //单项资源
        $d = $this->setTemplateAllocationInfo($d,10,$session_singleSource,'single_source_name',$tourPrice,$tourCurrency);
        //自费项目
        $d = $this->setTemplateAllocationInfo($d,11,$session_Optional,'own_expense_name',$tourPrice,$tourCurrency);
        ...
        if($settlement_type==1){
            foreach($OneBitePrice as $ky=>$OneBitePriceV){
                ...
                $ar['total_price'] = $OneBitePriceV;
                $ar['team_price_currency_id'] = $OneBitePriceCurrency[$ky]; //一口价货币
                $d['route_once_price'][] = $ar;
                unset($ar);
            }
        }
        $d['choose_company_id'] = session('user')['company_id'];
        //end 190327
...
//        var_dump($d);exit;
        ...
        if ($return['code'] == 200)
        {
            ...
            {
                $branch_array['user_company_id'] = $v;
                $route_template = $this->callSoaErp('post','/product/getRouteTemplate',['route_template_id' => $return['data'], 'can_watch_company_id' => $v])['data'][0];
...
//        exit;
     ...
     * @param $tourCurrency
      * @return mixed 返回写入数据
     */
    public function setTemplateAllocationInfo($d,$supplier_type_id,$source_data,$source_ky,$tourPrice,$tourCurrency){
        foreach($source_data as $vl){
            $ar['supplier_type_id'] = $supplier_type_id;
            $ar['source_id'] = $vl[$source_ky];
//            $ar['payment_currency_id'] = $vl['currency'];
...
//            $RouteType = Arrays::group($list['data'],'type');
            $this->assign('RouteType',$list['data']);
        }
...
//        var_dump($ReturnReceiptList);exit;
        ...
        //可见分公司
        $w['status'] = 1;
        $CompanyList = $this->callSoaErp('post','/system/getCompany',$w);
        if($CompanyList['data']){
    ...
    public function showTemplateHotelAdd(){
        //酒店供应商
        $data['status'] = 1;
        $data['supplier_type_id'] = 1;
        $data['company_id'] = session('user')['company_id'];
        //$data['supplier_type_id'] = 1;
        $List =  $this->callSoaErp('post','/source/getSupplier',$data);
        $data['status'] = 1;
        $data['supplier_type_id'] = 2;
        $data['company_id'] = session('user')['company_id'];
        //$data['supplier_type_id'] = 1;
        $SupplierList =  $this->callSoaErp('post','/source/getSupplier',$data);
        $arr = array_merge($SupplierList['data'],$List['data']);
        $this->assign('SupplierList',$arr);
        $this->getCurrency2();
...
//        var_dump($sessionTemplateHotelAjax);exit;
        //获取供应商
        $a = $this->getOneTypeSupplier(1);
        $b = $this->getOneTypeSupplier(2);
        $SupplierList = array_merge($a,$b);
...
//        return $this->fetch('new_template_hotel_update');
    ...
    public function showTemplateDiningAdd(){
        ...
        $data['company_id'] = session('user')['company_id'];
        $List =  $this->callSoaErp('post','/source/getSupplier',$data);
        $arr = array_merge($List['data'],$SupplierList['data']);
        $this->assign('SupplierList',$arr);
...
//        echo '<pre>';print_r($RouteTemplate['data'][0]);exit;
...
//            $RouteType = Arrays::group($list['data'],'type');
        ...
        $liuyan = $this->callSoaErp('post','/product/getRouteTemplateComment',$liuyan_params);
    
        $this->assign('comment_result',$liuyan['data']);
...
//         var_dump($session_hotel);exit;
    ...
    public function showRouteTemplateManage(){
        Cookie::set("route_template_manage",$_SERVER['REQUEST_URI']);
        //搜索
        $data = [
            'page'=>$this->page(),
        ...
        $route_user = input('get.route_user');
        ...
        if(!empty($route_type_id)){
            $data['route_type_id'] = $route_type_id;
        }
        if(!empty($route_user)){
            $data['route_user'] = $route_user;
        }
...
//        var_dump($data);exit;
        ...
        $this->assign('routeType',$RouteType['data']);
        $this->assign('source_type', $this->types);
...
//            $RouteType = Arrays::group($list['data'],'type');
    ...
    public function profitStatement(){
        ...
        $TeamProductReceivableSupplier = $this->callSoaErp('post','/product/getTeamProductReceivableSupplier',$where);

        if($TeamProductReceivableSupplier['data']){
            $TeamProductReceivableSupplier['data'] = Arrays::sort($TeamProductReceivableSupplier['data'],'source_type_id','asc');
            foreach($TeamProductReceivableSupplier['data'] as $k=>$v){
                $vl['团费']['count'] = count($TeamProductReceivableSupplier['data']);
                $d2 = Arrays::group($TeamProductReceivableSupplier['data'],'source_type_id');
                foreach($d2 as $ky=>$vy){
                    $vl['团费']['data'][$ky]['count'] = count($vy);
                    $vl['团费']['data'][$ky]['data'] = Arrays::group($vy,'payment_object_id');
                }
            }
        }
        $this->assign('TeamProductReceivableSupplierList',$vl);
        unset($where);
        unset($vl);

        //收分公司
        $number = input('get.plan_id'); //团队编号
        $where['team_product_id'] = $number;
        ...
        if($getTeamProductReceivableCompany['data']['receivable_info']){
            foreach($getTeamProductReceivableCompany['data']['receivable_info'] as $k=>$v){
                $getTeamProductReceivableCompany['data']['receivable_info'][$k]['payment_object_id'] = $getTeamProductReceivableCompany['data']['receivable_info'][$k]['payment_object_id']?:-1;
            }
        ...
        $this->assign('TeamProductReceivableCompany',$vl);
        unset($where);
        unset($vl);

        //直客/代理
        $TeamProductReceivableCustomerAndDistributor = $this->callSoaErp('post','/product/getTeamProductReceivableCustomerAndDistributor',['team_product_id'=>$number]);
        if($TeamProductReceivableCustomerAndDistributor['data']['receivable_info']){
            $receivable_info = $TeamProductReceivableCustomerAndDistributor['data']['receivable_info'];
            foreach($receivable_info as $k=>$v){
                if(empty($receivable_info[$k]['payment_object_type'])){
                    $receivable_info[$k]['payment_object_type'] = 'a';
                }
            }
            $receivable_info = Arrays::sort($receivable_info,'payment_object_type','asc');
            foreach($receivable_info as $k=>$v){
                $vl['团费']['count'] = count($receivable_info);
                $d2 = Arrays::group($receivable_info,'payment_object_type');
                foreach($d2 as $ky=>$vy){
                    $vl['团费']['data'][$ky]['count'] = count($vy);
                    $vl['团费']['data'][$ky]['data'] = Arrays::group($vy,'order_number');
                    foreach($vl['团费']['data'][$ky]['data'] as $k1=>$y1){
...
//            echo '<pre>';print_r($vl);exit;
        ...
        $this->assign('TeamProductReceivableCustomerAndDistributorlist',$vl);
        unset($w); unset($vl);
...
//        应付供应商
        ...
        $TeamProductCopeSupplierList = $vl;
        $this->assign('TeamProductCopeSupplierList',$vl);
        $this->assign('travel_agency_reimbursement_number',$travel_agency_reimbursement_number);
        unset($vl);
        //应付分公司
        $plan_id = input('get.plan_id');
        $TeamProductCopeCompany = $this->callSoaErp('post','/product/getTeamProductCopeCompany',['team_product_id'=>$plan_id]);
        if($TeamProductCopeCompany['data']){
            $TeamProductCopeCompany['data'] = Arrays::sort($TeamProductCopeCompany['data']['cope_info'],'receivable_object_id','asc');
//            echo '<pre>';print_r($TeamProductCopeCompany);exit;
            foreach($TeamProductCopeCompany['data'] as $k=>$v) {
                $vl['团费']['count'] = count($TeamProductCopeCompany['data']);
                $d2 = Arrays::group($TeamProductCopeCompany['data'], 'receivable_object_id');
                foreach ($d2 as $ky => $vy) {
                    $vl['团费']['data'][$ky]['count'] = count($vy);
                    $vl['团费']['data'][$ky]['data'] = Arrays::group($vy, 'order_number');
                    foreach($vl['团费']['data'][$ky]['data'] as $k1=>$y1){
...
//        echo '<pre>';print_r($vl);exit;
        $this->assign('TeamProductCopeCompanyList',$vl);

        //获取直客代理订单
//        $w['team_product_number'] = input('get.number');
//        $w['company_id'] = session('user')['company_id'];
        $w['status'] = 1;
        $w['team_product_id'] =  $number;
        $list = $this->callSoaErp('post','/branchcompany/getCompanyOrderNumberByTeamProductId',$w);
        $ar = [];
        foreach($list['data'] as $k=>$v){
            $ar[$k]['order_number'] = $v['company_order_number'];
        }
        $this->assign('order_number',$ar);
        unset($w);

        //分公司下的订单
        $where['team_product_id'] = $number;
        $Receivable = $this->callSoaErp('post','/branchcompany/getCompanyOrderNumberByTeamProductId',$where);
        ...
        $this->assign('team_product_result',$team_product_result['data'][0]);
        $this->assign('types',$this->types);
        $this->assign('types2',$this->types2);
        $this->assign('types3',['a'=>'index_public_select',14=>'index_product_public_groupFeeProject',11=>'index_finance_showReceivableManage_ownexpense']);


//        echo '<pre>';print_r($TeamProductCopeSupplierList);exit;
        return $this->fetch('n_plan_profit_statement');

    }


    //利润表
    public function profitStatement2(){
...
//        var_dump($param);
        ...
        $d['team_product_id'] = Arrays::get($param,'plan_id');
        $d['cope_info'] = [];
...
//        var_dump($param);exit;
        ...
        $where['system_alert_event_id'] = 12;
        $where['company_id'] = session('user')['company_id'];
        $where['team_product_id'] = $d['team_product_id'];
        $where['team_product_number'] = Arrays::get($param,'number');
        $this->callSoaErp('post','/system_alert_event/addInStationLetterAndEmail',$where);
    ...
    public function PlanSuppliersPayable(){
        ...
        $w['team_product_id'] = input('get.plan_id');
        $getTeamProductBase = $this->callSoaErp('post','/product/getTeamProductBase',$w);
        $this->assign('getTeamProductBase',$getTeamProductBase['data'][0]);
        unset($w);
...
//        var_dump($SupplierArr);exit;
    ...
    public function getReceivableAjax(){
        ...
        $w['status'] = 1;
        $w['team_product_id'] = input('plan_id');
...
//                    foreach ($team_coping_sn[$ak][$bk] as $ck => $cy) {
                    foreach ($team_coping_item[$ak][$bk] as $ck => $cy) {
//                        if($payment_company_id!='a' && $order_number!='a' && !empty($cy) &&  $team_coping_types[$ak][$bk][$ck]!='a'  && !empty($team_coping_item[$ak][$bk][$ck]) && $team_coping_currency[$ak][$bk][$ck]!='a' && !empty($team_coping_price[$ak][$bk][$ck]) ){
                        if($payment_company_id!='a' && $order_number!='a' &&  $team_coping_types[$ak][$bk][$ck]!='a'  && !empty($team_coping_item[$ak][$bk][$ck]) && $team_coping_currency[$ak][$bk][$ck]!='a' && !empty($team_coping_price[$ak][$bk][$ck]) ){
                            ...
                            $ar['company_order_customer_id'] = implode(',', $team_coping_sn[$ak][$bk][$ck]); //公司顾客ID
                            ...
                            $where['system_alert_event_id'] = 13;
                            $where['company_id'] = session('user')['company_id'];
                            $where['team_product_number'] = $d['team_product_number'];
                            $where['company_order_number'] = $order_number;
                            $this->callSoaErp('post','/system_alert_event/addInStationLetterAndEmail',$where);
                            unset($ar);
                        }
                    }
...
//                            $where1['system_alert_event_id'] = 13;
//                            $where1['company_id'] = session('user')['company_id'];
//                            $where1['team_product_number'] = $d['team_product_number'];
//                            $where1['company_order_number'] = $order_number;
//                            $this->callSoaErp('post','/system_alert_event/addInStationLetterAndEmail',$where1);
                            unset($ar);
                        }
                    }
                }
...
//            echo '<pre>'; print_r($getTeamProductReceivableCompany['data']['receivable_info']);exit;
...
//        print_r($vl['其他']);exit;
    ...
    public function PlanBooking(){
        ...
        if($_GET['search_book_queding']){
            $where['company_order_status'] = $_GET['search_book_queding'];
        }

//        var_dump($where);

        $order = $this->callSoaErp('post','/branchcompany/getCompanyOrderNumberByTeamProductId',$where);
...
//        var_dump($order);exit;
    ...
    public function PlanBookingInfo(){
        ...
        $where['status'] = 1;
        $where['team_product_id'] =input('plan_id');
        ...
        if(!empty($orderInfo['data'])){
            ...
            if(!empty($orderInfo['data']['company_order_product_team'])){
                foreach($orderInfo['data']['company_order_product_team'] as $k=>$v){
                    $orderInfo['data']['company_order_product_team'][$k]['customer_info_number'] = implode(',',Arrays::keys(Arrays::group($v['customer_info'],'company_order_customer_id')));
                    $orderInfo['data']['company_order_product_team'][$k]['customer_info_name'] = implode(',',array_column($v['customer_info'],'customer_name'));
                    $orderInfo['data']['company_order_product_team'][$k]['count'] = count($v['customer_info']);
                }
            ...
            if(!empty($orderInfo['data']['company_order_product_source'])){
                ...
                if($company_order_product_source[11]){
                    ...
                    foreach($company_order_product_source_11 as $k=>$v){
                        $company_order_product_source_11[$k]['customer_info_number'] = implode(',',Arrays::keys(Arrays::group($v['customer_info'],'company_order_customer_id')));

                        $company_order_product_source_11[$k]['customer_info_name'] = implode(',',array_column($v['customer_info'],'customer_name'));
                        $company_order_product_source_11[$k]['count'] = count($v['customer_info']);
                    }
            ...
            foreach($CompanyOrderCustomer['data'] as $k=>$v){
                ...
                if($CompanyOrderCustomer['data'][$k]['flight_info']){
                    foreach($CompanyOrderCustomer['data'][$k]['flight_info'] as $ky=>$vy){
                        ...
                        $CompanyOrderCustomer['data'][$k]['flight_info'][$ky]['flight_begin_time'] = $CompanyOrderCustomer['data'][$k]['flight_info'][$ky]['flight_begin_time']?date('Y-m-d H:i',$CompanyOrderCustomer['data'][$k]['flight_info'][$ky]['flight_begin_time']):'-';
                        $CompanyOrderCustomer['data'][$k]['flight_info'][$ky]['flight_end_time'] = $CompanyOrderCustomer['data'][$k]['flight_info'][$ky]['flight_end_time']?date('Y-m-d H:i',$CompanyOrderCustomer['data'][$k]['flight_info'][$ky]['flight_end_time']):'-';
                    }
...
////        var_dump($CompanyOrderCustomer['data'][3]['flight_info']);exit;
        ...
        $comment_result  = $this->callSoaErp('post', '/branchcompany/getCompanyOrderComment', $comment_params);
        ...
        $this->assign('room_type',json_encode($this->room_type));
...
//        return $this->fetch('plan_booking_passenger');
        ...
        //获取酒店资源
        $data['company_id'] =  session('user')['company_id'];
        $getHotel = $this->callSoaErp('post','/source/getHotel',$data);
        $this->assign('getHotel',$getHotel['data']);
...
//        echo '<pre>';print_r($param);exit;
        ...
        $d['country_id'] = Arrays::get($param,'country_id')?:0;
        $d['birthday'] = Arrays::get($param,'birthday')?strtotime(Arrays::get($param,'birthday')):'';
        $d['language_id'] = Arrays::get($param,'language_id')?:0;
//        $d['customer_type'] = Arrays::get($param,'customer_type');
        $d['phone'] = Arrays::get($param,'phone');
        $d['issuing_date'] = Arrays::get($param,'issuing_date')?strtotime(Arrays::get($param,'issuing_date')):'';
        $d['term_of_validity'] = Arrays::get($param,'term_of_validity')?strtotime(Arrays::get($param,'term_of_validity')):'';
        ...
        $d['special_claim'] = Arrays::get($param,'special_claim');
        //住宿信息
        ...
        if($d['check_in']<0){
            $in = [];
            $check_in = $d['check_in']*-1;
            for($i=1;$i<=$check_in;$i++){
              $in[] =  $check_in_edit[$i];
            }
            $d['check_in_hotel'] = implode(',',$in);
        }
        if($d['check_on']>0){
            $on = [];
            $check_on = $d['check_on'];
            for($i=1;$i<=$check_on;$i++){
                $on[] = $check_on_edit[$i];
            }
            $d['check_on_hotel'] = implode(',',$on);
        }
...
//        $d['check_on_hotel'] = implode(',',$check_on_edit);
        ...
        foreach($flight_code as $k=>$v){
            ...
            $ary['flight_end_time'] = $flight_end_time[$k]?strtotime($flight_end_time[$k]):'';
            $ary['flight_type'] = $flight_type[$k]?:'';
            $ary['start_place'] = $start_place[$k]?:'';
            $ary['end_place'] = $end_place[$k]?:'';
            $ary['remark'] = $remark[$k]?:'';
            $d['customer_flight'][] = $ary;
            unset($ary);
        }

        $where['system_alert_event_id'] = 8;
        $where['company_id'] = session('user')['company_id'];
        $where['company_order_customer_id'] = $d['company_order_customer_id'];
        $this->callSoaErp('post','/system_alert_event/addInStationLetterAndEmail',$where);
...
//        var_dump($param);
    ...
    public function updateCompanyOrderCustomerStatusByCompanyOrderCustomerId(){
        ...
        $where['system_alert_event_id'] = 8;
        $where['company_id'] = session('user')['company_id'];
        $where['company_order_customer_id'] = $up['company_order_customer_id'];
        $this->callSoaErp('post','/system_alert_event/addInStationLetterAndEmail',$where);
    ...
    public function BatchConfirmationAjax(){
        ...
        foreach($ar as $v){
            ...
            $where['system_alert_event_id'] = 8;
            $where['company_id'] = session('user')['company_id'];
            $where['company_order_customer_id'] = $company_order_customer_id;
            $this->callSoaErp('post','/system_alert_event/addInStationLetterAndEmail',$where);
    ...
    public function PlanSubgroupAjax(){
        ...
        //获取其他团号
        $w['team_product_id'] = input('post.team_product_id');
        $OtherRegimentNumber = $this->callSoaErp('post','/product/otherRegimentNumber',$w);
        $ay['data']['OtherRegimentNumber'] = $OtherRegimentNumber['data']?:[];
    ...
    public function ConfirmPlanSubgroupAjax(){
        ...
        $w['old_team_product_id'] = Arrays::get($param,'old_team_product_id');
        $w['order_number'] = Arrays::get($param,'bookingId');
        $w['new_team_product_id'] = -1;
        $rFentuan = Arrays::get($param,'r-fentuan');
        if($rFentuan==2){
            $w['new_team_product_id'] = Arrays::get($param,'n_team_product_id');
        }
        $this->callSoaErp('post','/product/subGroup/',$w);
    ...
    public function ShowPlanTour(){
        Cookie::set("plan_tour_manage",$_SERVER['REQUEST_URI']);
    	$this->_http_referer;
        //线路类型
//        $cateList = $this->callSoaErp('post','/system/getRouteType',$data);
//        if($cateList['data']){
...
//        $setPlanTourManage = Session::get('setPlanTourManage');
        $setPlanTourManage = Request::instance()->param();
//        var_dump($setPlanTourManage);exit;
       // echo is_numeric($setPlanTourManage['search_plur_status']);
        $where = [
            'page'=>$this->page(),
            'page_size'=>$this->_page_size,
        ];
        ...
        if(is_numeric($setPlanTourManage['is_profit_approve'])){  //团队名称
        	$where['is_profit_approve'] = $setPlanTourManage['is_profit_approve'];
        }
        if($setPlanTourManage['search_responsible_person']){ //线路负责人
            $where['team_product_user'] = $setPlanTourManage['search_responsible_person'];
        }
        if($setPlanTourManage['search_route_type']){ //类型
            $where['route_type_id'] = $setPlanTourManage['search_route_type'];
        }

        if(is_numeric($setPlanTourManage['status']) ){ //状态
            $where['status'] = $setPlanTourManage['status'];
        }

        if($setPlanTourManage['search_all_time_name']){ //出团日期
            $where['begin_time'] = strtotime($setPlanTourManage['search_all_time_name']);
            
        }
        if($setPlanTourManage['search_all_time_name2']){ //出团日期
        	$where['end_time'] = strtotime($setPlanTourManage['search_all_time_name2']);
        }
        if(!empty($setPlanTourManage['choose_company_id'])){         //所属分公司
            $where['choose_company_id'] = $setPlanTourManage['choose_company_id'];
        }else{
        	$where['choose_company_id'] = session('user')['company_id'];
        }

        if(is_numeric(input('search_plur_status'))){ //团队状态
            $where['plur_status'] = $setPlanTourManage['search_plur_status'];
        }

        if(!empty($setPlanTourManage['customer_confirm'])){ //游客已确认
            $where['customer_confirm'] = $setPlanTourManage['customer_confirm'];
        }
		
        if(!empty($setPlanTourManage['company_order_status'])){ //游客已确认
        	$where['company_order_status'] = $setPlanTourManage['company_order_status'];
        }     
        
        $where['is_between_time'] = 1;
        $where['is_like'] = 1;
        $where['can_watch_company_id'] = session('user')['company_id'];
...
//        echo '<pre>';print_r($TeamProductList);exit;
        ...
        $team_statu = [
        	'0'=>[
        		'tag_name'=>'index_product_showRouteOwnExpenseAdd_no_conglomerate',
        		'plur_status'=>0	
        	],
        	'1'=>[
        		'tag_name'=>'index_product_plan_begin',
        		'plur_status'=>0.5        			
        	],
        	'2'=>[
        		'tag_name'=>'index_product_showRouteOwnExpenseAdd_conglomerate',
        		'plur_status'=>1
        	],
        	'3'=>[
        		'tag_name'=>'index_product_showRouteOwnExpenseAdd_close_an_account',
        		'plur_status'=>2
        	],
        	'4'=>[
        		'tag_name'=>'index_product_showRouteOwnExpenseAdd_close_group',
        		'plur_status'=>3
        	]
        ...
        $this->assign('team_statu',$team_statu);
        ...
        $this->assign('plur_status',$setPlanTourManage['search_plur_status']);
        $this->assign('company_result',$company_result['data']);
        $this->assign('source_type', $this->types);
        $this->assign('choose_company_id',$where['choose_company_id']);
        $this->assign('customer_confirm',$where['customer_confirm']);
...
//        var_dump($param);exit;
        ...
        $time_zone = Arrays::get($param,'time_zone'); //线路模板
        ...
        foreach($dateData as $dateDataV){
            ...
            $d['company_id'] = session('user')['company_id'] ;
            $d['operational_log'] = Arrays::get($param,'operational_log');
...
//        var_dump($session_singleSource);exit;
    ...
    public function getOneTypeSupplier($supplier_type_id=0){
        $where['status']=1;
        $where['company_id'] = session('user')['company_id'];
        if($supplier_type_id){
            $where['supplier_type_id']=$supplier_type_id;
...
//        $time_zone = Arrays::get($post,'time_zone');
//        $d['time_zone'] = strtotime($time_zone);
        ...
        $d['user_id'] = session('user')['user_id'];
        $d['company_id'] = session('user')['company_id'] ;
        $d['operational_log'] = Arrays::get($post,'operational_log');
...
//var_dump($session_Optional);
        ...
        $data1['system_alert_event_id'] = 4;
        $data1['team_product_id'] = $team_product_id;
        $data1['company_id'] = session('user')['company_id'];
        if(session('user')['company_id']==1){
            $data1['choose_company_id'] = session('user')['company_id'];
        }
...
//        return $this->fetch('plan_ptional_add');
    ...
    public function showProductQuotationHtmlInfo(){
        ...
        $route_template_id = Arrays::get($post,'route_template_id',0);
...
//                    var_dump($TeamProductPrice);exit;
            ...
            if($route_template_id){
                $where['route_settlement_type'] = 1;
                $where['route_template_id'] = $route_template_id;
                $where['status'] = 1;
                $TeamProductPrice = $this->callSoaErp('post','/product/getRouteOnePrice',$where);
                if($TeamProductPrice['data']){
                    $TeamProductPrice = Arrays::group($TeamProductPrice['data'],'company_id');
                    $this->assign('TeamProductPrice',$TeamProductPrice);
                }
            ...
            if(empty($use_company_id)){
                ...
                foreach($use_company_id as $k=>$v){
                    $use_company_id[$k]['val'] = $v['value'];
                }
            }
            $this->assign('use_company_id',$use_company_id);
//            var_dump($use_company_id); exit;
...
//            print_r($zt_OwnExpense_resource);exit;
    ...
    * 线路模板产品报价
     */
    public function showTemplateQuotationHtml(){
        $post = Request::instance()->param();
        $settlement_type = Arrays::get($post,'settlement_type');
        $use_company_id = Arrays::get($post,'use_company_id');
        $plan_id = Arrays::get($post,'route_template_id',0);
        $this->assign('settlement_type',$settlement_type);

        $session_hotel = Session::get('sessionTemplateHotelAjax');
        $session_dining = Session::get('sessionTemplateDiningAjax');
        $session_flight = Session::get('sessionTemplateFlightAjax');
        $session_cruise = Session::get('sessionTemplateCruiseAjax');
        $session_visa = Session::get('sessionTemplateVisaAjax');
        $session_scenic_spot = Session::get('sessionTemplateScenicSpotAjax');
        $session_vehicle = Session::get('sessionTemplateVehicleAjax');
        $session_tourGuide = Session::get('sessionTemplateTourGuideAjax');
        $session_singleSource = Session::get('sessionTemplateSingleSourceAjax');
        $session_Optional = Session::get('sessionTemplateOptionalAjax');
...
//        var_dump($resource);exit;
        ...
        if($settlement_type==1){
            if($plan_id){
                $where['route_settlement_type'] = 1;
                $where['route_template_id'] = $plan_id;
                $where['status'] = 1;
                $TeamProductPrice = $this->callSoaErp('post','/product/getRouteOnePrice',$where);
//                var_dump($TeamProductPrice);exit;
                if($TeamProductPrice['data']){
...
//            print_r($zt_OwnExpense_resource);exit;
        ...
        return $this->fetch('template_quotation');
    }
    ...
    public function getCustomerInfoAjax(){
        ...
        if($cc == "hotel"){
            ...
            foreach($customer_data['data'] as $key=>$val){
                $val['room_type'] = $roomType[$val['room_type']];
...
//            var_dump($customer_data);exit;
    ...
    public function showRouteTypeManage(){
        Cookie::set("route_type_manage",$_SERVER['REQUEST_URI']);
        //搜索
        $data = [
            'page'=>$this->page(),
            'page_size'=>$this->_page_size,
        ...
        $p_id = $this->callSoaErp('post', '/system/getOneRouteType',$where);
    ...
    public function addRouteTypeAjax(){
        ...
        $route_type_code = input("route_type_code");
        $data = [
    ...
    public function showRouteTypeEdit(){
        ...
        $data = ["route_type_id"=>$route_type_id];
        $route_type_result = $this->callSoaErp('post', '/system/getOneRouteType',$data);
        $this->assign('route_type_result',$route_type_result['data']);
        ...
        $data1['status'] = 1;
        $data1['company_id'] = session('user')['company_id'];
        $cateList = $this->callSoaErp('post','/system/getRouteType',$data1);
        $cateList = Help::toData($cateList['data'],$route_type_result['data']['route_type_id']);
        ...
        $this->assign('company_result',$company_result['data']);
        //操作用户
        $this->assign('UserId',$route_type_result['data']['route_type_user_id']);
    ...
    public function showRouteTypeInfo(){
        ...
        if($cateList){
            $this->assign('cateList', array_column($cateList, 'route_type_id'));
        }
        //获取公司信息
        $data2 = ['status'=>1];
        $company_result =  $this->callSoaErp('post', '/system/getCompany',$data2);
    ...
    public function showReturnReceiptManage(){
        ...
        if(!empty($setReturnReceiptManage['search_status'])){
            $data['status'] = $setReturnReceiptManage['search_status'] ? : 1;
        }
        $data['company_id'] =  session('user')['company_id'];
    ...
    public function addShowReturnReceiptAjax(){
        ...
        $data['type'] = Arrays::get($post,'type');
     ...
     * Time: 10:43
     */
    public function getRouteTemplateAjax(){
        $data['status'] = 1;
        $data['route_number'] = input("route_number");
        $data['route_name'] = input("route_name");
        $data['route_user'] = input('route_user');
        $data['company_id'] =  input("company_id");
        $data['can_watch_company_id'] =  input("route_name");

        $data['not_company_id'] =  input("not_company_id");
        $data['can_watch_company_id'] =  input("can_watch_company_id");
    ...
    public function upPlurStatusAjax(){
        ...
        $d['plur_status'] = input('post.plur_status');
        if($d['plur_status']==1){
            $w['team_product_number'] = input('post.u_team_product_number');
            $w['team_product_id'] = input('post.u_team_product_id');
            $this->callSoaErp('post','/product/addTeamProductPriceAndCope',$w); //成团


            $where['system_alert_event_id'] = 11;
            $where['company_id'] = session('user')['company_id'];
            $where['team_product_id'] = input('post.u_team_product_id');
            $this->callSoaErp('post','/system_alert_event/addInStationLetterAndEmail',$where);

        }
        $d['team_product_id'] = input('post.u_team_product_id');
        $d['user_id'] = session('user')['user_id'];


        $dy = $this->callSoaErp('post','/product/upPlurStatusAjax',$d);

        if($dy['code']==200){
            $dy['plur_status'] = $d['plur_status'];

        }else {
            $dy['plur_status'] = $d['plur_status'] - 1;
        }
        return $dy;
    }
    ...
    /**
    * 线路模板同步团队产品
    * 线路模板同步团队产品operations
     */
    public function synchronizationTeamProduct(){
        $d['route_template_id'] = input('route_template_id');
        $d['s_time'] = input('ss_time');
        $d['e_time'] = input('ee_time');
        $d['user_id'] = session('user')['user_id'];
        return $this->callSoaErp('post','/product/synchronizationTeamProduct',$d);
    }
    ...
    * 线路模板获取团队产品出团日期
    ...
    public function selTeamProductbyRouteTemplateId(){
        ...
        $r = $this->callSoaErp('post','/product/selTeamProductbyRouteTemplateId',$d);
        return $r;
    ...
    */
    public function tourStatistics(){
        ...
        $s_time = Arrays::get($post,'s_time');
        $e_time = Arrays::get($post,'e_time');
        $route_user = Arrays::get($post,'route_user');
        $route_type_id = Arrays::get($post,'route_type_id');

        //分公司信息
        $w['status'] = 1;
        $company = $this->callSoaErp('post','/system/getCompany',$w);
        $this->assign('company',$company['data']);
        unset($w);

        if($s_time){
            $w['s_time'] = strtotime($s_time);
        }
        if($e_time){
            $w['e_time'] = strtotime($e_time);
        }
        if($route_user){
            $w['route_user'] = $route_user;
        }
        if($route_type_id){
            $w['route_type_id'] = $route_type_id;
        }
        $w['company_id'] = session('user')['company_id'];
        $arr = $this->callSoaErp('post','/product/getNumberOfPassengersOnALine',$w);
        $route_type = $arr['data']['route_type'];
        $route_template = $arr['data']['route_template'];
        $NumberOfPassengersOnALine = $arr['data']['NumberOfPassengersOnALine'];



        $NumberOfPassengersOnALine_G = Arrays::group($NumberOfPassengersOnALine,function($ar){ //分公司||线路模板
           return $ar['company_id'].'||'.$ar['route_template_id'];
        });
        $NumberOfPassengersOnALine_route_template_id = Arrays::group($NumberOfPassengersOnALine,'route_template_id'); // 根据线路模板分类
        $NumberOfPassengersOnALine_route_type_id = Arrays::group($NumberOfPassengersOnALine,'route_type_id'); // 根据线路类型
        $NumberOfPassengersOnALine_companyId_routeTypeId = Arrays::group($NumberOfPassengersOnALine,function($ar){ //分公司||线路类型
            return $ar['company_id'].'||'.$ar['route_type_id'];
        });


//        var_dump($NumberOfPassengersOnALine_route_type_id);exit;

        $this->assign('route_type',$route_type);
        $this->assign('route_template',Arrays::group($route_template,'route_type_id'));
        $this->assign('NumberOfPassengersOnALine',$NumberOfPassengersOnALine);
        $this->assign('route_type_group',Arrays::group($route_type,'route_type_id'));
        $this->assign('NumberOfPassengersOnALine_G',$NumberOfPassengersOnALine_G);
        $this->assign('NumberOfPassengersOnALine_route_template_id',$NumberOfPassengersOnALine_route_template_id);
        $this->assign('NumberOfPassengersOnALine_route_type_id',$NumberOfPassengersOnALine_route_type_id);
        $this->assign('NumberOfPassengersOnALine_companyId_routeTypeId',$NumberOfPassengersOnALine_companyId_routeTypeId);

        return $this->fetch('tour_statistics');
    ...
    public function supplierReceivable(){
        ...
        $where['team_product_id'] = input('get.plan_id');
        $TeamProductReceivableSupplier = $this->callSoaErp('post','/product/getTeamProductReceivableSupplier',$where);
        //echo '<pre>';print_r($TeamProductReceivableSupplier);exit;
...
//         ];
//         $result = $this->callSoaErp('post','/product/getStatementApprove',$statement_approve_params);
...
//         $result = $result['data'][0]['json_data'];
//     	$result = json_decode($result,true);
//     	$result = $result['receivable_supplier_array'];
...
//     	$this->assign('result',$result);
        ...
        $this->assign('types',$this->types);
        return $this->fetch('plan_supplier_receivable');
    }

    //执行修改应收供应商
    public function addSupplierReceivableAjax(){
        $param = Request::instance()->param();
        $data['receivable_supplier_array'] = [];
        $team_coping_type = Arrays::get($param,'team_coping_type');
        $team_coping_supplier = Arrays::get($param,'team_coping_supplier');
        $team_coping_item = Arrays::get($param,'team_coping_item');
        $team_coping_currency = Arrays::get($param,'team_coping_currency');
        $team_coping_price = Arrays::get($param,'team_coping_price');
        $team_coping_quantity = Arrays::get($param,'team_coping_quantity');
        $team_coping_cope_money = Arrays::get($param,'team_coping_cope_money');
        $team_coping_number = Arrays::get($param,'team_coping_number');

        $data['team_product_id'] = Arrays::get($param,'plan_id');
        foreach($team_coping_type as $ky=>$vy){
            $source_type_id = $vy;
            if(is_numeric($vy)){
                foreach($team_coping_supplier[$ky] as $ku=>$vu){
                    $payment_object_id = $vu;
                    foreach($team_coping_item[$ky][$ku] as $k=>$v){
                        //应收编号
                        if($team_coping_number[$ky][$ku][$k]){
                            $ary['receivable_number'] = $team_coping_number[$ky][$ku][$k]; //应付编号
                        }

                        $ary['source_type_id'] = $source_type_id;
                        $ary['payment_object_id'] = $payment_object_id;
                        $ary['product_name'] = $team_coping_item[$ky][$ku][$k];
                        $ary['receivable_currency_id'] = $team_coping_currency[$ky][$ku][$k];
                        $ary['price'] = $team_coping_price[$ky][$ku][$k];
                        $ary['unit'] = $team_coping_quantity[$ky][$ku][$k];
                        $ary['receivable_money'] = $team_coping_cope_money[$ky][$ku][$k];
                        $data['receivable_supplier_array'][] = $ary;
                        unset($ary);
                    }
                }
            }
...
//        error_log(print_r($data,1));
        return $this->callSoaErp('post','/product/addTeamProductReceivableSupplier',$data);
    }


    //应付分公司
    public function payableBranch(){
        $plan_id = input('get.plan_id');
        $TeamProductCopeCompany = $this->callSoaErp('post','/product/getTeamProductCopeCompany',['team_product_id'=>$plan_id]);
//        echo '<pre>';print_r($TeamProductCopeCompany);exit;
        //获取团队产品当前状态
...
//        var_dump($ReceivableList);exit;
        ...
        return $this->fetch('plan_payable_branch');
    }


    //执行修改应付分公司
    public function addPayableBranchAjax(){
        $param = Request::instance()->param();
        $data['cope_info_array'] = [];
        $team_coping_branch = Arrays::get($param,'team_coping_branch');
        $team_coping_booking = Arrays::get($param,'team_coping_booking');
        $team_coping_types = Arrays::get($param,'team_coping_types');
        $team_coping_sn = Arrays::get($param,'team_coping_sn');
        ...
        $cope_number = Arrays::get($param,'cope_number');

        foreach($team_coping_branch as $ak=>$ay){
            if (is_numeric($ay)) {
                $payment_object_id = $ay;
                foreach($team_coping_booking[$ak] as $bk=>$by){
                    $order_number = $by;
                    foreach($team_coping_types[$ak][$bk] as $ck=>$cy){
                        if($payment_object_id!='a' && $order_number!='a' &&  $team_coping_types[$ak][$bk][$ck]!='a'  && !empty($team_coping_item[$ak][$bk][$ck]) && $team_coping_currency[$ak][$bk][$ck]!='a' && !empty($team_coping_price[$ak][$bk][$ck]) ){
                           if($cope_number[$ak][$bk][$ck]){
                               $ar['cope_number'] = $cope_number[$ak][$bk][$ck];
                           }

                            $ar['payment_object_id'] = $payment_object_id;
                            $ar['order_number'] = $order_number;
                            $ar['company_order_customer_id'] = implode(',',$team_coping_sn[$ak][$bk][$ck]);
                            $ar['source_type_id'] = $team_coping_types[$ak][$bk][$ck];
                            $ar['product_name'] = $team_coping_item[$ak][$bk][$ck];
                            $ar['cope_currency_id'] = $team_coping_currency[$ak][$bk][$ck];
                            $ar['cope_money'] = $team_coping_price[$ak][$bk][$ck];
                            $data['cope_info_array'][] = $ar;
                            unset($ar);
                        }
                    }
                }
        ...
        $data['team_product_id'] = Arrays::get($param,'plan_id');
        return $this->callSoaErp('post','/product/addTeamProductCopeCompany',$data);
    }
	/**
	 * 团队产品直接下单
	 */
    public function buyTeamProduct(){
    	$param = Request::instance()->param();
    	$team_product_id = $param['plan_id'];
    	//首先调用订单 基础 接口
    	...
    	$company_order_result = $this->callSoaErp('post','/branchcompany/addCompanyOrder',$company_order_params);


    	if($company_order_result['code']==200){//继续插入数据
    		...
    		$company_order_product_params = [
    				'company_order_number'=>$company_order_result['data'],
    				'team_product_array'=>[
    					['team_product_id'=>$team_product_id,'route_template_number'=>$team_product_result['data'][0]['route_number']]


    				]
    		];

    		$this->callSoaErp('post','/branchcompany/addCompanyOrderProduct',$company_order_product_params);
    		$this->redirect('/branchcompany/companyOrderManage?company_order_number='.$company_order_result['data']);
    ...
    public function copyCircuitTemplate(){
       $d['route_template_id'] = input('post.route_template_id');
       $d['user_id'] = session('user')['user_id'];
       return  $this->callSoaErp('post','/product/copyCircuitTemplate/',$d);
   }

    //收-直客、代理
    public function ReceivableDirectPassengerAgent(){
        $plan_id = input('get.plan_id');
        ...
        $TeamProductReceivableCustomerAndDistributor = $this->callSoaErp('post','/product/getTeamProductReceivableCustomerAndDistributor',['team_product_id'=>$plan_id]);
//        echo '<pre>';print_r($TeamProductReceivableCustomerAndDistributor);exit;
...
//        var_dump($ReceivableList);exit;
        ...
        $this->assign('types',['a'=>'index_public_select',14=>'index_product_public_groupFeeProject',11=>'index_finance_showReceivableManage_ownexpense']);


       return $this->fetch('receivable_direct_passenger_agent');
    }

    //添加 收-直客/代理
    public function addReceivableDirectPassengerAgentAjax(){
        $param = Request::instance()->param();
...
//        echo '<pre>';print_r($param);exit;
        ...
        $team_coping_currency = Arrays::get($param,'team_coping_currency');
        $team_coping_price = Arrays::get($param,'team_coping_price');
        $receivable_number = Arrays::get($param,'receivable_number');
        $data['team_product_id'] = Arrays::get($param,'plan_id');
        $data['receivable_info'] = [];
        foreach($team_coping_branch as $k1=>$v1){
            foreach($team_coping_booking[$k1] as $k2=>$v2){
                $order_number = $v2;
                foreach($team_coping_types[$k1][$k2] as $k3=>$v3){
                    if(!empty($receivable_number[$k1][$k2][$k3])){
                        $ar['receivable_number'] = $receivable_number[$k1][$k2][$k3];
                    }
                    $ar['order_number'] = $order_number;
                    $ar['company_order_customer_id'] = implode(',',$team_coping_sn[$k1][$k2][$k3]);
                    $ar['source_type_id'] = $team_coping_types[$k1][$k2][$k3];
                    $ar['product_name'] = $team_coping_item[$k1][$k2][$k3];
                    $ar['receivable_currency_id'] = $team_coping_currency[$k1][$k2][$k3];
                    $ar['receivable_money'] = $team_coping_price[$k1][$k2][$k3];
                    $data['receivable_info'][] = $ar;
                    unset($ar);
                }
            }
        }

        return $this->callSoaErp('post','/product/addTeamProductReceivableCustomerAndDistributor',$data);
    ...
    public function getStatementApproveAjax(){
    	 
    	$params = Request::instance()->param();
    
    
    	$result = $this->callSoaErp('post', '/product/getStatementApprove', $params);
    	$result = $result['data'][0]['json_data'];
    	$result = json_decode($result,true);
    	$result = $result['receivable_supplier_array'];
	 ...
	 * 获取应收供应商ajax
	 */
    public function getTeamProductReceivableSupplierAjax(){
    	
    	$params = Request::instance()->param();
    	 

    	$result = $this->callSoaErp('post', '/product/getTeamProductReceivableSupplier', $params);
     ...
     * 结算单审核
     */
    public function postStatementApproveAjax(){
    	$params = Request::instance()->param();
    
    	$result = $this->callSoaErp('post', '/product/postStatementApprove', $params);
    	 
    	return $result;
    }
    //修改团队产品 根据团队产品ID
    public function updateTeamProductByTeamPdocutId(){
    	$params = Request::instance()->param();
    	
    	$result = $this->callSoaErp('post', '/product/updateTeamProductBaseByTeamProductBaseId', $params);
     ...
     */
    public function addRouteTemplateCommentAjax()
    {
    	$params = Request::instance()->param();
    	...
    	$result = $this->callSoaErp('post', '/product/addRouteTemplateComment', $params);
    	
    	$data = [
    			 
    			'system_alert_event_id'=>29,
    ...
    //回执单
   public function tourGuideIframe(){
        	$number =input('number');
          ...
          return $this->fetch('tourGuideIframe');
         }
         
         //添加订单留言板
    public function addTeamProductCompanyOrderCommentAjax()
         {
         	$params = Request::instance()->param();
