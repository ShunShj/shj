...
//        setcookie("search['currency_manage']",$_SERVER['REQUEST_URI'],time()+3600);
//        setcookie("search['123']",$_SERVER['REQUEST_URI'],time()+3600);
        Cookie::set("currency_manage",$_SERVER['REQUEST_URI']);
        $data = [
        	'page'=>$this->page(),
   ...
   */
    ...
    */
    ...
    public function showUserAdd(){
        ...
        $lianyi_email = Contents::getNexusEmail();
        

    	$this->assign('company_data_result',$company_data_result['data']);   
    	$this->assign('language_data_result',$language_data_result['data']);
        $this->assign('role_data_result',$role_data_result['data']);
        $this->assign('lianyi_email',$lianyi_email);
    ...
    public function editUserAjax(){
        ...
        //获取语言标签
//         $language_result =  $this->callSoaErp('post','/system/getTagByLanguageId',['company_id' => $user['company_id'], 'language_id' => $data['language_id']]);

//         if ($language_result['code'] == 200)
//         {
//             Help::languageReplace($language_result['data'], $user['language_id'], $user['user_id']);
//         }
...
//         	'status'=>1	
//         ];
//         $result = $this->callSoaErp('post', '/system/getCodeIsTrue',$system_params);
//         $result = $result['data'];
//        	if(count($result)==0){//没有新的验证码
//        		return '没有验证码/no code';
//        	}else{
// 			if($result[0]['code']==input('code')){
// 				 $this->callSoaErp('post', '/user/updateUserByUserId',$data);
// 				$updateEmailSms = [
// 					'status'=>2,
// 					'email_sms_id'=>$result[0]['email_sms_id']	
// 				];
// 				$this->callSoaErp('post', '/system/updateEmailSms',$updateEmailSms);
// 				return 1;
// 			}else{
// 				return '验证码错误/code is error';//验证码错误
// 			}
//        	}
        exit();
...
//    	}
    ...
    public function showCurrencyProportionManage(){
        ...
        if(empty($time)){
        	...
        	$date_time = date('Ym',strtotime('- 1 month'));
        }else{
        	$date_time = $time;
...
//         }else{
    ...
    */
	 ...
	 * 获取国家区域AJAX
	 */
	public function getCountryCityAjax(){
		$params = Request::instance()->param();
		$result = $this->callSoaErp('post', '/system/getCountryCity', $params);
    ...
    public function sendSystemEmailAjax()
    {
    	$data = Request::instance()->param();
    	
    
    	$result = $this->callSoaErp('post', '/system/sendSystemEmail',$data);
    ...
    public function showRoomTypeManage(){
        ...
        $result = $this->callSoaErp('post', '/system/getRoomType',$data);

        $this->getPageParams($result);
        return $this->fetch('room_type_manage');
    }


    public function showRoomTypeAdd(){

        return $this->fetch('room_type_add');
    }


    public function addRoomTypeAjax(){

        $data = Request::instance()->param();

        $data['user_id'] = session('user')['user_id'];

        $result = $this->callSoaErp('post', '/system/addRoomType',$data);

        return $result;

    }

    public function showRoomTypeEdit(){
        
        $data['room_type_id'] = input('room_type_id');
        $result = $this->callSoaErp('post', '/system/getOneRoomType',$data);

        $this->assign('result',$result['data']);

        return $this->fetch('room_type_edit');
    }

    public function editRoomTypeAjax(){

        $data = Request::instance()->param();

        $data['user_id'] = session('user')['user_id'];

        $result = $this->callSoaErp('post', '/system/updateRoomType',$data);

        return $result;

    }

    public function delRoomTypeAjax(){

        $post = Request::instance()->param();
        $data['room_type_id'] = Arrays::get($post,'room_type_id');
        return $this->callSoaErp('post','/system/deleteRoomType',$data);
    }
    ...
    public function showSeasonManage(){
        ...
        $result = $this->callSoaErp('post', '/system/getSeason',$data);

        $this->getPageParams($result);
        return $this->fetch('season_manage');
    }


    public function showSeasonAdd(){

        return $this->fetch('season_add');
    }


    public function addSeasonAjax(){

        $data = Request::instance()->param();

        $data['user_id'] = session('user')['user_id'];

        $result = $this->callSoaErp('post', '/system/addSeason',$data);

        return $result;

    }

    public function showSeasonEdit(){

        $data['season_id'] = input('season_id');
        $result = $this->callSoaErp('post', '/system/getOneSeason',$data);

        $this->assign('result',$result['data']);

        return $this->fetch('season_edit');
    }

    public function editSeasonAjax(){

        $data = Request::instance()->param();

        $data['user_id'] = session('user')['user_id'];

        $result = $this->callSoaErp('post', '/system/updateSeason',$data);

        return $result;

    }

    public function delSeasonAjax(){

        $post = Request::instance()->param();
        $data['season_id'] = Arrays::get($post,'season_id');
        return $this->callSoaErp('post','/system/deleteSeason',$data);
    }
    ...
    public function getSeasonAjax(){
        $params = Request::instance()->param();
        $result = $this->callSoaErp('post', '/system/getSeasonAjax', $params);
