...
//        var_dump($param);
	...
	public function showReceivableManage(){
        ...
        $order_data_result = $this->callSoaErp('post', '/branchcompany/getCompanyOrder',$order_data);
        //获取货币
        $currency_data = [
			'status'=>1
        ];
...
//         $customer_number = trim($customer_number,',');
    ...
    public function addReceivableInfoAjax(){
		...
		for($i=0;$i<count($receivable_info_array);$i++){
			...
			$params['payment_money'] =  substr($receivable_info_array[$i],strpos($receivable_info_array[$i],'_')+1);
			$receivable[] = $params;
        ...
        $data = [
        	...
        	'receivable_voucher'=>$invoice_number,	
	...
	public function showMustPayManage(){
        ...
        //error_log(print_r(help::modelDataToArr($cope_result),1));
	...
	public function showMustPayAllManage(){
        ...
        $cope_number = input('cope_number');
        $product_name = input('team_product_name');
        $supplier_name = input('supplier_name');
        if(!empty($cope_number)){
            $cope_data['cope_number'] = $cope_number;
        }
        if(!empty($product_name)){
            $cope_data['team_product_name'] =$product_name;
        }
        if(!empty($supplier_name)){
	...
	public function addCopeInfoAjax(){
		...
		for($i=0;$i<count($cope_info_array);$i++){
			...
			$params['payment_money'] =  substr($cope_info_array[$i],strpos($cope_info_array[$i],'_')+1);
			$cope[] = $params;
	...
	public function showAgentExpenseManage(){
        ...
        //error_log(print_r(help::modelDataToArr($data),1));
