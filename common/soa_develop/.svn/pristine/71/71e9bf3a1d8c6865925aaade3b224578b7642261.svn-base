...
use think\config;
use think\Db;
class RouteSingleSource extends Model{
    //protected $connection = ['database' => 'erp'];
    protected $table = 'route_single_source';
     ...
     * 胡
     */
    public function addRouteSourceVisa($params){
    	$t = time();

    	$data['route_template_id'] = $params['route_template_id'];
    	$data['visa_id'] = $params['visa_id'];
    	...
    	Db::startTrans();
    	try{
    		$pk_id = Db::name('route_source_visa')->insertGetId($data);
