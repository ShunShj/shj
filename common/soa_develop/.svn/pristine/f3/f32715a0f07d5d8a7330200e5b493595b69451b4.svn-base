...
use think\config;
use think\Db;
class RouteSourceCruise extends Model{
    //protected $connection = ['database' => 'erp'];
    protected $table = 'route_source_cruise';
     ...
     * 胡
     */
    public function addRouteSourceCruise($params){
    	$t = time();

    	$data['route_template_id'] = $params['route_template_id'];
    	$data['cruise_id'] = $params['cruise_id'];
    	...
    	Db::startTrans();
    	try{
    		$pk_id = Db::name('route_source_cruise')->insertGetId($data);
     ...
     * 修改回执单
     */
    public function updateRouteSourceByRouteSourceId($params){
    	...
    	$data['cruise_id'] = $params['cruise_id'];
    	$data[''] = $params['source_price'];
    	$data[''] = $params['source_count'];
