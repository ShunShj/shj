...
use think\config;
use think\Db;
class RouteSourceScenicSport extends Model{
    //protected $connection = ['database' => 'erp'];
    protected $table = 'route_source_scenic_sport';
     ...
     * 胡
     */
    public function addRouteSourceScenicSport($params){
    	$t = time();

    	$data['route_template_id'] = $params['route_template_id'];
    	$data['scenic_spot_id'] = $params['scenic_spot_id'];
    	...
    	Db::startTrans();
    	try{
    		$pk_id = Db::name('route_source_scenic_sport')->insertGetId($data);
